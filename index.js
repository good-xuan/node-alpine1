...
{
  createWriteStream
} = require('fs'); // 引入 createWriteStream
...
{
  pipeline
} = require('stream/promises'); // 引入原生流式管道
...
const State =
{
  ...
  async save(data)
  {
    ...
    await fs.writeFile(CONFIG.PERSIST_FILE, JSON.stringify(
      {
        ...current,
        ...data
      }
...
(async() =>
{
  ...
  try
  {
    ...
    if (CONFIG.ENABLE_XRAY)
    {
      ...
      {
        stdout: findOut
      } = await execAsync(`find ${TMP} -type f -name "xray" | head -n 1`);
      ...
      else
      {
        try
        {
          ...
          {
            stdout
          } = await execAsync(`${FILES.BIN} tls cert`,
          {
            encoding: 'utf-8'
          }
            );
      ...
      if (CONFIG.ENABLE_PQ && (!keys.decryption || !keys.encryption))
      {
        try
        {
          ...
          {
            stdout
          } = await execAsync(`${FILES.BIN} vlessenc`,
          {
            encoding: 'utf-8'
          }
            );
